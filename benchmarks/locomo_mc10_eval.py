#!/usr/bin/env python3
"""
LoCoMo-MC10 Benchmark Evaluation for Veld

This script evaluates Veld on the LoCoMo-MC10 benchmark,
...
Requirements:
    pip install datasets tqdm veld
...
# HTTP API Client for veld server
class VeldMemoryClient:
    """HTTP client for veld server API."""

    def __init__(self, base_url: str = "http://127.0.0.1:3030", api_key: str = "sk-veld-dev-local-testing-key", user_id: str = "locomo-eval"):
...
def store_conversations(
    client: VeldMemoryClient,
    sessions: list,
    summaries: list[str],
    datetimes: list[str] = None
) -> tuple[int, float]:
    """Store conversation sessions into Veld via HTTP API.
...
def recall_context(client: VeldMemoryClient, question: str, limit: int = 5) -> tuple[str, float]:
...
def run_evaluation(
    ...
    veld_url: str = "http://127.0.0.1:3030",
    veld_api_key: str = "sk-veld-dev-local-testing-key",
    output_file: str = "locomo_results.json"
):
    """Run the full LoCoMo-MC10 evaluation."""

    print("=" * 60)
    print("LoCoMo-MC10 Benchmark for Veld")
    print("=" * 60)

    # Create HTTP client for veld
    print(f"\nConnecting to Veld at {veld_url}")
    client = VeldMemoryClient(base_url=veld_url, api_key=veld_api_key)

    # Test connection
    try:
        resp = requests.get(f"{veld_url}/health")
        if resp.status_code != 200:
            print(f"ERROR: Veld server not responding at {veld_url}")
            exit(1)
        print("[OK] Connected to Veld server")
    except Exception as e:
        print(f"ERROR: Cannot connect to Veld server: {e}")
        exit(1)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="LoCoMo-MC10 Benchmark for Veld",
...
Examples:
  ...
  python locomo_mc10_eval.py --provider ollama --model llama3.2 --limit 100
    ...
    parser.add_argument("--veld-url", default="http://127.0.0.1:3030", help="Veld server URL")
    parser.add_argument("--veld-api-key", default="sk-veld-dev-local-testing-key", help="Veld API key")
    ...
    run_evaluation(
        ...
        limit=limit,
        veld_url=args.veld_url,
        veld_api_key=args.veld_api_key,
