#!/usr/bin/env python3
"""
LongMemEval Benchmark Evaluation for veld

Runs the official LongMemEval benchmark (ICLR 2025, 500 questions, 6 types)
against veld's retrieval pipeline. Produces reproducible, publishable
...
Requirements:
    ...
    # veld server must be running on localhost:3030
...
class VeldMemoryClient:
    def __init__(self, base_url="http://127.0.0.1:3030",
                 api_key="sk-veld-dev-local-testing-key"):
...
def ingest_question(client: VeldMemoryClient, entry: dict,
                    user_id: str, granularity: str = "turn") -> dict:
    """Ingest all haystack sessions for one question into veld."""
...
def run_evaluation(args):
    ...
    # Health check
    client = VeldMemoryClient(args.veld_url, args.api_key)
    if not client.health():
        print(f"ERROR: veld server not reachable at {args.veld_url}")
        print("Start with: cargo run -- serve")
        sys.exit(1)
    print(f"veld server OK at {args.veld_url}")
...
def print_summary(output: dict):
    ...
    print("\n" + "=" * 70)
    print("  LongMemEval Results — veld v0.5.0")
    print("=" * 70)
...
def main():
    parser = argparse.ArgumentParser(
        description="LongMemEval benchmark for veld")
    ...
    parser.add_argument("--veld-url", default="http://127.0.0.1:3030")
    parser.add_argument("--api-key", default="sk-veld-dev-local-testing-key")
