...
// Get context from veld
export async function getMemoryContext(userMessage: string): Promise<string> {
  try {
    const response = await fetch(`${config.veld.apiUrl}/api/proactive_context`, {
      method: "POST",
      headers: {
...
export async function rememberConversation(userMessage: string, response: string): Promise<void> {
  try {
    await fetch(`${config.veld.apiUrl}/api/remember`, {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
        "X-API-Key": config.veld.apiKey,
...
- [SEND_FILE:path] - Sends a file to Telegram, e.g. [SEND_FILE:C:/Users/Private Citizen/Desktop/notes.txt]
- [LIST_DIR:path] - Lists directory contents, e.g. [LIST_DIR:C:/Users/Private Citizen/Desktop]
