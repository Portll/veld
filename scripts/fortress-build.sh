#!/usr/bin/env bash
#
# fortress-build.sh — Fractal binary obfuscation pipeline for veld
#
...
#   target/fortress/veld                (obfuscated CLI binary)
#
set -euo pipefail
...
for bin in "${BINARIES[@]}"; do
    ...
    ENVVARS=$(strings "$bin" 2>/dev/null | grep -c 'VELD_' || true)
    if [[ $ENVVARS -gt 0 ]]; then
        echo -e "    ${RED}✗ LEAK: $ENVVARS VELD_ env var strings found${NC}"
        strings "$bin" | grep 'VELD_' | head -5 | while read -r line; do
            echo -e "      ${RED}→ $line${NC}"
        done
        LEAKED=$((LEAKED + ENVVARS))
    else
        echo -e "    ${GREEN}✓ No VELD_ env var strings leaked${NC}"
    fi
