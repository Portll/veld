...
Hooks fire for ALL tool types: Edit, Write, MultiEdit, Bash, Read, Task, Search, Grep, Glob, ListFiles, WebFetch, NotebookEdit. Context is surfaced pre-tool and actions are recorded post-tool.
...
- `hooks/memory-hook.ts` — hook implementation (all 6 lifecycle events, all tool types)
- User ID: `claude-code`
...
- `hooks/claude-settings.json` — portable config using `$VELD_HOOKS_DIR` env var
