...
///
/// System-level only. This route is unauthenticated (Kubernetes probe), so it
/// MUST NOT disclose per-tenant existence or statistics. Per-user readiness
/// belongs on an authenticated `/api/health/*` route — see SEC remediation plan.
