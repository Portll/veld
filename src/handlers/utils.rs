...
/// Removes <system-reminder>, <veld-context>, Claude Code system prompts, and code blocks.
pub fn strip_system_noise(content: &str) -> String {
    ...
    // Remove <veld-context>...</veld-context> blocks (our own injected context)
    while let Some(start) = result.find("<veld-context") {
        if let Some(end) = result.find("</veld-context>") {
            let end_pos = end + "</veld-context>".len();
            if end_pos <= result.len() && start < end_pos {
                result = format!("{}{}", &result[..start], &result[end_pos..]);
            } else {
                break;
    ...
    // Remove <veld>...</veld> blocks (hook-injected context that shouldn't be re-ingested)
    while let Some(start) = result.find("<veld") {
        if let Some(end) = result.find("</veld>") {
            let end_pos = end + "</veld>".len();
            if end_pos <= result.len() && start < end_pos {
                result = format!("{}{}", &result[..start], &result[end_pos..]);
            } else {
                break;
