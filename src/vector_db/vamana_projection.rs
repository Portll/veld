...
//!
//! `Memory` carries up to five embedding kinds (primary, secondary,
//! image, audio, video). Vamana — today — indexes only ONE kind. We pick
//! `experience.embeddings` (the *primary* text embedding) because that is
//! the field `RetrievalEngine::index_memory` already feeds into Vamana on
//! the live path; using a different kind here would mean the projection
//! disagreed with the existing read path. Memories that arrive with
//! `embeddings: None` are logged at warn level and skipped — Vamana
//! cannot index an unembedded memory, but the apply must still report
//! success so the LSN checkpoint advances and replay doesn't loop on
//! that record forever.
//!
//! Multi-embedding-kind support (secondary 768d, multimodal) is tracked
//! as a follow-up — it requires either a per-kind Vamana per user (the
//! `RetrievalEngine` already does this with `secondary_vector_index`) or
//! a multi-vector node type.
//!
//! ## Idempotency
//!
...
/// migration.
pub const PROJECTION_NAME: &str = "vamana";
...
///
/// One per tenant. Either holds an already-constructed
/// `Arc<RwLock<VamanaIndex>>` (the eager variant — caller knows the
/// dim) OR a `VamanaProjectionBootstrap` that builds the index on the
/// first embedded memory it sees (the lazy variant — caller doesn't
/// know the dim yet because no embedder has been opened for this tenant).
/// Holds a shared [`CheckpointStore`] for per-projection LSN
/// bookkeeping, an in-memory `memory_id → u32` map, and an in-memory
/// copy of the last-applied LSN.
pub struct VamanaProjection {
    ...
    /// startup the replay driver rebuilds it by walking the log. We
    /// don't persist it separately — that's the whole point of the
    /// projection being log-derived.
...
impl VamanaProjection {
    /// Construct a projection around an already-open Vamana index. Use
    /// this when the caller knows the embedding dimension up-front
    /// (e.g. they read it off the embedder cache before opening the
    /// projection). Reads the last-persisted checkpoint from
    /// `checkpoint_store` so subsequent `replay` / live `apply` calls
    /// resume from the right LSN.
    pub fn new(
    ...
    /// Construct a projection that will lazily build its Vamana index
    /// from the first embedded memory it sees. Useful when the
    /// projection is opened before any embedder is — typically at server
    /// startup, where the per-tenant `RetrievalEngine` is constructed on
    /// first request, not at boot.
    pub fn lazy(
    ...
    fn apply_typed(
        ...
        match payload {
            ...
            | IntentPayload::Update {
                ...
                let embedding = match memory.experience.embeddings {
                    Some(emb) => emb,
                    None => {
                        // Memory has no primary embedding — nothing to
                        // feed Vamana. This is not a hard error: the
                        // RocksDB write succeeded, the SQLite projection
                        // captured the body, and a later re-embedding
                        // pass will eventually populate `embeddings`.
                        // Advance the checkpoint so replay doesn't loop
                        // on this frame forever.
                        tracing::warn!(
                            memory_id = %memory_id,
                            lsn = lsn.0,
                            "vamana projection: skipping memory without primary embedding (no-op apply; checkpoint advances)",
                        );
                        self.checkpoint = Some(self.bumped_checkpoint(lsn));
                        return Ok(());
                    }
...
mod tests {
    ...
    fn open_projection() -> (
        ...
        let proj = VamanaProjection::new(index.clone(), ckpt.clone());
    ...
    fn replay_from_scratch_rebuilds_full_state_from_log() {
        ...
        let index = Arc::new(RwLock::new(VamanaIndex::new(test_config()).unwrap()));
        let ckpt = Arc::new(Mutex::new(CheckpointStore::open(&ckpt_path).unwrap()));
        let mut proj = VamanaProjection::new(index.clone(), ckpt);
    ...
    fn restart_resumes_from_persisted_checkpoint() {
        ...
        // First boot: apply only LSN 0..=2 via the static Projection arm.
        {
            ...
            let ckpt = Arc::new(Mutex::new(CheckpointStore::open(&ckpt_path).unwrap()));
            let mut proj = VamanaProjection::new(index, ckpt);
    ...
    fn lazy_projection_materialises_index_on_first_embedded_memory() {
        ...
        let mut proj = VamanaProjection::lazy(bootstrap, ckpt);
