...
impl NerConfig {
    ...
    pub fn from_env() -> Self {
        let base_path = std::env::var("VELD_NER_MODEL_PATH")
            .map(PathBuf::from)
            .unwrap_or_else(|_| {
                ...
                let candidates: Vec<Option<PathBuf>> = vec![
                    ...
                    dirs::data_dir().map(|p| p.join("veld/models/bert-tiny-ner")),
                ];
        ...
        let confidence_threshold = std::env::var("VELD_NER_CONFIDENCE")
            .ok()
