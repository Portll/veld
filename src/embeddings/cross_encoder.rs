...
//!
//! Auto-downloads to ~/.cache/shodh-memory/cross-encoder/ on first use.
...
impl CrossEncoder {
    ...
    fn ensure_downloaded() -> Result<(PathBuf, PathBuf)> {
        ...
        // Check offline mode
        if std::env::var("SHODH_OFFLINE").is_ok_and(|v| v == "true" || v == "1") {
            anyhow::bail!(
                "Cross-encoder model not found and SHODH_OFFLINE=true. \
