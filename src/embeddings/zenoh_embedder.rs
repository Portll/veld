...
impl ZenohEmbedderConfig {
    ...
    pub fn from_env() -> Self {
        Self {
            prefix: std::env::var("VELD_ZENOH_EMBED_PREFIX")
                .unwrap_or_else(|_| "veld".into()),
            target_model: std::env::var("VELD_ZENOH_EMBED_MODEL")
                .unwrap_or_else(|_| "minilm-l6-v2".into()),
            dimension: std::env::var("VELD_ZENOH_EMBED_DIMENSION")
                .ok()
                .and_then(|s| s.parse().ok())
                .unwrap_or(384),
            timeout_ms: std::env::var("VELD_ZENOH_EMBED_TIMEOUT_MS")
                .ok()
                .and_then(|s| s.parse().ok())
                .unwrap_or(5000),
            connect_endpoints: std::env::var("VELD_ZENOH_CONNECT")
                .map(|s| {
                    s.split(',')
                ...
                .unwrap_or_default(),
            api_key: std::env::var("VELD_ZENOH_API_KEY").ok(),
        }
    }
}
