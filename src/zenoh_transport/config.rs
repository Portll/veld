...
pub struct ZenohConfig {
    ...
    /// Key expression prefix for all veld topics.
    /// Default: `"veld"`.
    ///
...
impl ZenohConfig {
    ...
    pub fn from_env() -> Self {
        ...
        if let Ok(val) = std::env::var("VELD_ZENOH_ENABLED") {
            config.enabled = val == "true" || val == "1";
        }

        if let Ok(val) = std::env::var("VELD_ZENOH_MODE") {
        ...
        if let Ok(val) = std::env::var("VELD_ZENOH_CONNECT") {
            config.connect = val
                .split(',')
        ...
        if let Ok(val) = std::env::var("VELD_ZENOH_LISTEN") {
            config.listen = val
                .split(',')
                .map(|s| s.trim().to_string())
                .filter(|s| !s.is_empty())
        ...
        if let Ok(val) = std::env::var("VELD_ZENOH_PREFIX") {
        ...
        if let Ok(val) = std::env::var("VELD_ZENOH_AUTO_TOPICS") {
        ...
        if let Ok(val) = std::env::var("VELD_ZENOH_API_KEY") {
            let trimmed = val.trim().to_string();
            if !trimmed.is_empty() {
