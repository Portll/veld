"""
OpenAI Agents SDK Integration for Veld
...
Usage (Tools):
    from veld.integrations.openai_agents import VeldTools
    from agents import Agent, Runner

    tools = VeldTools(user_id="agent-1", api_key="your-key")
    agent = Agent(
        name="memory-agent",
        instructions="You have persistent memory. Use veld_remember to store and veld_recall to retrieve.",
        tools=tools.as_list(),
    )
    result = Runner.run_sync(agent, "Remember that I prefer Python over JavaScript")

Usage (Session):
    from veld.integrations.openai_agents import VeldSession
    from agents import Agent, Runner

    session = VeldSession(session_id="conv-123", user_id="agent-1", api_key="your-key")
...
# ---------------------------------------------------------------------------
# VeldTools — FunctionTool provider for OpenAI Agents SDK
# ---------------------------------------------------------------------------

class VeldTools(_VeldHTTPClient):
    """OpenAI Agents SDK tools backed by Veld.

    Provides 8 tools for persistent memory and task management:
    - veld_remember: Store memories with types and tags
    - veld_recall: Semantic/hybrid search across memories
    - veld_forget: Delete a specific memory
    - veld_context_summary: Get condensed overview of recent learnings/decisions
    - veld_proactive_context: Auto-surface relevant memories for current context
    - veld_add_todo: Create tasks with projects, priorities, contexts
    - veld_list_todos: List tasks filtered by status/project
    - veld_complete_todo: Mark a task as complete
...
# ---------------------------------------------------------------------------
# VeldSession — Session protocol implementation
# ---------------------------------------------------------------------------

class VeldSession(_VeldHTTPClient):
    """OpenAI Agents SDK Session backed by Veld.
