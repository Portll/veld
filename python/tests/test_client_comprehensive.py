...
VeldError = CLIENT_MODULE.VeldError
VeldConnectionError = CLIENT_MODULE.VeldConnectionError
VeldAuthenticationError = CLIENT_MODULE.VeldAuthenticationError
VeldValidationError = CLIENT_MODULE.VeldValidationError
VeldNotFoundError = CLIENT_MODULE.VeldNotFoundError
VeldRateLimitError = CLIENT_MODULE.VeldRateLimitError
VeldServerError = CLIENT_MODULE.VeldServerError
...
class TestDefaultStoragePath(unittest.TestCase):
    ...
    def test_linux_xdg_path(self, mock_system, mock_exists):
        with mock.patch.dict(os.environ, {"XDG_DATA_HOME": "/custom"}, clear=False):
            result = _default_storage_path()
            self.assertEqual(result, "/custom/veld")
...
class TestHandleResponseError(unittest.TestCase):
    ...
    def test_429_raises_rate_limit(self):
        ...
        with self.assertRaises(VeldRateLimitError) as ctx:
            _handle_response_error(r)
    ...
    def test_500_raises_server_error(self):
        ...
        with self.assertRaises(VeldServerError) as ctx:
            _handle_response_error(r)
        self.assertEqual(ctx.exception.status_code, 500)

    def test_503_raises_server_error(self):
        r = FakeResponse(503, json_data={"error": "unavailable"})
        with self.assertRaises(VeldServerError):
            _handle_response_error(r)

    def test_unknown_status_raises_veld_error(self):
        r = FakeResponse(418, json_data={"error": "teapot"})
        with self.assertRaises(VeldError):
            _handle_response_error(r)
...
class TestExceptions(unittest.TestCase):
    def test_validation_error_with_field(self):
        e = VeldValidationError("bad input", field="content")
        self.assertEqual(e.field, "content")

    def test_not_found_error_attributes(self):
        e = VeldNotFoundError("memory", "abc-123")
...
class TestMemoryInit(unittest.TestCase):
    ...
    def test_reads_api_key_from_env(self, mock_ensure):
        with mock.patch.dict(os.environ, {"VELD_API_KEY": "env-key"}):
...
class TestMemoryAdd(unittest.TestCase):
    ...
    def test_add_connection_error(self):
        ...
        with self.assertRaises(VeldConnectionError):
            mem.add("test")
...
class TestPlatformDetection(unittest.TestCase):
    def test_get_binary_name_darwin(self):
        ...
        with mock.patch("platform.system", return_value="Darwin"):
            self.assertEqual(mem._get_binary_name(), "veld-darwin")

    def test_get_binary_name_linux(self):
        mem = _build_memory()
        with mock.patch("platform.system", return_value="Linux"):
            self.assertEqual(mem._get_binary_name(), "veld")

    def test_get_binary_name_windows(self):
        mem = _build_memory()
        with mock.patch("platform.system", return_value="Windows"):
            self.assertEqual(mem._get_binary_name(), "veld.exe")

    @mock.patch("pathlib.Path.exists", return_value=False)
    @mock.patch("shutil.which", return_value=None)
    def test_find_binary_not_found(self, mock_which, mock_exists):
        mem = _build_memory()
        result = mem._find_binary("veld")
        self.assertIsNone(result)

    @mock.patch("shutil.which", return_value="/usr/local/bin/veld")
    @mock.patch("pathlib.Path.exists", return_value=False)
...
class TestStartServer(unittest.TestCase):
    def test_start_success(self):
        ...
        mock_path.__str__ = lambda self: "/usr/bin/veld"
        with mock.patch.object(mem, "_get_binary_name", return_value="veld"):
