#!/bin/bash
# session-start.sh — Veld session start hook for veld project
# Runs on every SessionStart event (new, resume, compact, clear).
# Auto-starts the veld-server binary if down.
...
# Source veld-lib (sets VELD_URL, VELD_API_KEY, VELD_USER_ID)
source "$SCRIPT_DIR/veld-lib.sh" 2>/dev/null || true

# ── Auto-Start Server if Down ─────────────────────────────────
if ! curl -sf --max-time 2 "$VELD_URL/health" > /dev/null 2>&1; then
  # Prefer release binary, fall back to debug
  VELD_BIN=""
  for candidate in \
    "$PROJECT_DIR/target/release/veld-server" \
    "$PROJECT_DIR/target/debug/veld-server"; do
    if [ -x "$candidate" ]; then
      VELD_BIN="$candidate"
      break
    fi
  ...
  if [ -n "$VELD_BIN" ]; then
    VELD_API_KEYS="$VELD_API_KEY" nohup "$VELD_BIN" \
      --port 3030 \
      --storage "$PROJECT_DIR/veld_data" \
      > /tmp/veld.log 2>&1 &
    # Wait up to 8s for health
    for i in 1 2 3 4 5 6 7 8; do
      sleep 1
      curl -sf --max-time 1 "$VELD_URL/health" > /dev/null 2>&1 && break
    done
    # Re-check — reset VELD_HEALTHY so veld_health() re-probes
    VELD_HEALTHY=""
    veld_health 2>/dev/null || exit 0
  else
    exit 0
  fi
else
...
VERIFY_RESULT=$(curl -sf --max-time 5 -X POST "$VELD_URL/api/verify_index" \
  -H "Content-Type: application/json" \
  -H "X-API-Key: $VELD_API_KEY" \
  -d "{\"user_id\":\"$VELD_USER_ID\"}" 2>/dev/null)
...
" 2>/dev/null)
  if [ "${ORPHAN_COUNT:-0}" -gt "0" ]; then
    export VELD_INDEX_WARNING="[veld] Index desync: $ORPHAN_COUNT orphaned memories. Run repair_index to fix."
  fi
fi
...
TMPSUM="/tmp/veld_sum_$$.json"
TMPREL="/tmp/veld_rel_$$.json"
TMPTAGS="/tmp/veld_tags_$$.json"
...
curl -s --max-time 10 -X POST "$VELD_URL/api/context_summary" \
  -H "Content-Type: application/json" \
  -H "X-API-Key: $VELD_API_KEY" \
  -d "{\"user_id\":\"$VELD_USER_ID\",\"limit\":$SUM_LIMIT}" > "$TMPSUM" 2>/dev/null &

curl -s --max-time 15 -X POST "$VELD_URL/api/relevant" \
  -H "Content-Type: application/json" \
  -H "X-API-Key: $VELD_API_KEY" \
  -d "{\"user_id\":\"$VELD_USER_ID\",\"context\":\"$REL_CONTEXT\",\"limit\":$REL_LIMIT}" > "$TMPREL" 2>/dev/null &

curl -s --max-time 10 -X POST "$VELD_URL/api/recall/tags" \
  -H "Content-Type: application/json" \
  -H "X-API-Key: $VELD_API_KEY" \
  -d "{\"user_id\":\"$VELD_USER_ID\",\"tags\":[\"architecture\",\"retrieval\",\"memory-system\",\"regression\",\"struggle-file\",\"veld\",\"embeddings\",\"graph\",\"benchmark\",\"session-summary\"],\"limit\":$TAG_LIMIT}" > "$TMPTAGS" 2>/dev/null &

# Overlook: fetch prior AVs for cross-session learning
TMPOVERLOOK="/tmp/overlook-priors-$CLAUDE_AGENT_PID.json"
curl -s --max-time 5 -X POST "$VELD_URL/api/recall/tags" \
  -H "Content-Type: application/json" \
  -H "X-API-Key: $VELD_API_KEY" \
  -d "{\"user_id\":\"$VELD_USER_ID\",\"tags\":[\"overlook-av\",\"session-end\"],\"limit\":3}" \
...
git -C "$PROJECT_DIR" diff --name-only HEAD~3 2>/dev/null | head -15 | while IFS= read -r f; do
  [ -n "$f" ] && veld_file_patterns "$PROJECT_DIR/$f" 2>/dev/null
done

# ── Consolidation Trigger (every 10th session) ───────────────
COUNT_FILE="/tmp/veld_session_count_${CLAUDE_PROJECT_HASH:-default}"
COUNT=$(($(cat "$COUNT_FILE" 2>/dev/null || echo 0) + 1))
echo "$COUNT" > "$COUNT_FILE"
if [ $((COUNT % 10)) -eq 0 ]; then
  curl -s --max-time 5 -X POST "$VELD_URL/api/consolidation/report" \
    -H "Content-Type: application/json" -H "X-API-Key: $VELD_API_KEY" \
    -d "{\"user_id\":\"$VELD_USER_ID\"}" > /dev/null 2>&1 &
fi
...
lines = [f"=== VELD MEMORY (session: {source}) ==="]
...
result = "\n".join(lines)
index_warning = os.environ.get("VELD_INDEX_WARNING", "")
if index_warning:
