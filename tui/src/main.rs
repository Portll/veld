...
use logo::{ELEPHANT, ELEPHANT_FRAMES, GRASS_SUFFIX, VELD_GRADIENT, VELD_TEXT};
use stream::{
...
fn format_auth_message(message: &str) -> String {
    message
        .replace(
            ". Expected a key from VELD_API_KEYS or VELD_DEV_API_KEY.",
            ".\nExpected a key from VELD_API_KEYS or VELD_DEV_API_KEY.",
        )
...
fn default_api_key_path() -> PathBuf {
    dirs::config_dir()
        .map(|dir| dir.join("veld").join("config.toml"))
        .unwrap_or_else(|| PathBuf::from(".veld").join("config.toml"))
}
...
async fn run_statusline() -> Result<()> {
    let base_url = env::var("VELD_API_URL")
        .or_else(|_| env::var("VELD_SERVER_URL"))
    ...
    // POST to veld backend (fire and forget)
    let client = reqwest::Client::new();
...
async fn main() -> Result<()> {
    ...
    let base_url = std::env::var("VELD_SERVER_URL")
        ...
        .to_string();
    let mut api_key = std::env::var("VELD_API_KEY")
        .unwrap_or_else(|_| "sk-veld-dev-local-testing-key".to_string());
...
mod tests {
    ...
    fn auth_message_gets_expected_line_breaks() {
        let formatted = format_auth_message(
            "Invalid API key. Expected a key from VELD_API_KEYS or VELD_DEV_API_KEY. Default dev key: 'abc'",
        );
        assert!(formatted.contains("Invalid API key.\nExpected a key"));
        assert!(formatted.contains("VELD_DEV_API_KEY.\nDefault dev key"));
    }

    #[test]
    fn error_message_is_extracted_from_json_body() {
        let body = r#"{"code":"INVALID_API_KEY","message":"Invalid API key. Expected a key from VELD_API_KEYS or VELD_DEV_API_KEY. Default dev key: 'abc'"}"#;
