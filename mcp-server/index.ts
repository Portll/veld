...
const USER_ID = process.env.VELD_USER_ID || "claude-code";
...
// Set via VELD_API_KEY env var, or configure in MCP settings
let API_KEY = process.env.VELD_API_KEY || (SANDBOX_MODE ? "sandbox" : "");
if (!API_KEY) {
  if (isLocalServer()) {
    // Auto-generate a random key for local development — zero config
    API_KEY = crypto.randomBytes(32).toString("hex");
    console.error(
      "[veld] No API key set — auto-generated for local server.",
    );
  } else {
    console.error("ERROR: VELD_API_KEY is required for remote servers.");
    console.error("");
    console.error(
      "To fix, add to your MCP config (claude_desktop_config.json or mcp.json):",
    );
    console.error(`  "env": { "VELD_API_KEY": "your-api-key" }`);
    console.error("");
    console.error("Or set in your shell:");
    console.error("  export VELD_API_KEY=your-api-key");
    process.exit(1);
  }
}
...
if (shouldWarnInsecureApiUrl(API_URL, process.env.VELD_ALLOW_HTTP)) {
  console.error(
    "[veld] WARNING: Using HTTP for a non-localhost server is insecure.",
  );
  console.error(
    "[veld] Set VELD_API_URL to an https:// URL, or set VELD_ALLOW_HTTP=true to suppress this warning.",
  );
}
...
const TOKEN_BUDGET = parseInt(process.env.VELD_TOKEN_BUDGET || "100000", 10);
...
function stripSystemNoise(text: string): string {
  ...
  const tagPatterns = [
    ...
    /<veld-context[\s\S]*?<\/veld-context>/g,
    /<veld[\s\S]*?<\/veld>/g,
...
server.setRequestHandler(CallToolRequestSchema, async (request) => {
  ...
  const executeTool = async (): Promise<ToolResult> => {
    switch (name) {
      ...
      case "list_reminders": {
        ...
        let response = `🐘 VELD REMINDERS (${result.count})\n`;
        response += `━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n`;
  ...
  try {
    ...
    if (message.includes("ECONNREFUSED") || message.includes("fetch failed")) {
      ...
      helpText = "\n\nAuthentication failed. Check your VELD_API_KEY.";
...
// Read a specific resource (veld:// or memory://)
server.setRequestHandler(ReadResourceRequestSchema, async (request) => {
  const uri = request.params.uri;

  try {
    // Handle veld:// static resources
    if (uri.startsWith("veld://")) {
      const resource = uri.replace("veld://", "");
...
- **/mcp__veld__quick_recall** - Search memories
- **/mcp__veld__session_summary** - Session overview
- **/mcp__veld__what_i_know** - Everything about a topic
- **/mcp__veld__pending_work** - View todos
- **/mcp__veld__recent_memories** - Recent memories
- **/mcp__veld__memory_health** - System status
`;
          return {
...
// =============================================================================
// MCP PROMPTS - Discoverable commands via /mcp__veld__<name>
// =============================================================================
...
// Disable auto-spawn with VELD_NO_AUTO_SPAWN=true
const AUTO_SPAWN_ENABLED = process.env.VELD_NO_AUTO_SPAWN !== "true";
...
async function ensureServerRunning(): Promise<void> {
  ...
  if (await isServerRunning()) {
    console.error("[veld] Backend server already running at", API_URL);
    // If we auto-generated a key, verify it works against the running server
    if (!process.env.VELD_API_KEY && isLocalServer()) {
      const keyWorks = await validateApiKey();
      if (!keyWorks) {
        console.error(
          "[veld] WARNING: Auto-generated key rejected by running server.",
        );
        console.error(
          "[veld] The server was started with a different API key.",
        );
        console.error(
          "[veld] Set VELD_API_KEY to match the server's key, or restart",
        );
        console.error(
          "[veld] the server without VELD_DEV_API_KEY to use auto-generated keys.",
        );
      }
    }
    return;
  ...
  if (!AUTO_SPAWN_ENABLED) {
    console.error("[veld] Server not running at", API_URL);
    console.error(
      "[veld] Auto-spawn disabled (VELD_AUTO_SPAWN=false).",
    );
    console.error("[veld] Start the server manually:");
    console.error("[veld]   veld");
    console.error("[veld] Or with Docker:");
    console.error(
      "[veld]   docker run -d -p 3030:3030 roshera/veld",
    );
    return;
  }
  ...
  if (!binaryPath) {
    console.error(
      "[veld] Server binary not found. Please run: npx @veld/memory-mcp",
    );
    console.error(
      "[veld] Or download from: https://github.com/Portll/veld/releases",
    );
    return;
  }
  ...
  if (
    ...
    console.error(
      `[veld] WARNING: Binary path resolves outside expected directory: ${resolvedBinary}`,
    );
    console.error(`[veld] Expected: ${expectedBinDir}`);
    return;
  }

  console.error("[veld] Starting backend server...");

  // Build a clean environment for the server process.
  // Only pass through system env + server-relevant VELD_ vars.
  // MCP-client-specific vars (VELD_RATE_LIMIT, VELD_TOKEN_BUDGET, etc.)
  ...
  console.error("[veld] Waiting for server to start...");
  const started = await waitForServer();

  if (started) {
    console.error("[veld] Backend server started successfully");
  } else {
    console.error(
      "[veld] Warning: Server may not have started properly",
    );
  }
}
...
process.on("SIGINT", () => {
  console.error("[veld] Received SIGINT, shutting down...");
  cleanupServer();
  process.exit(0);
});

process.on("SIGTERM", () => {
  console.error("[veld] Received SIGTERM, shutting down...");
  cleanupServer();
  process.exit(0);
});
...
async function main() {
  ...
  console.error("Veld MCP server v0.7.7 running");
  console.error(`Connecting to: ${API_URL}`);
  console.error(`User ID: ${USER_ID}`);
  console.error(`Streaming: ${STREAM_ENABLED ? "enabled" : "disabled"}`);
  console.error(
    `Proactive surfacing: ${PROACTIVE_SURFACING ? "enabled" : "disabled (VELD_PROACTIVE=false)"}`,
  );
}
