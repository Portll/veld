#!/bin/bash
# Veld Auto-Ingest Hook for Claude Code
#
# This hook runs after each Claude Code response to automatically
# store conversations in veld for persistent learning.
#
# Installation:
...
#   VELD_API_URL - API endpoint (default: http://127.0.0.1:3030)
#   VELD_API_KEY - API key (default: dev key)
#   VELD_USER_ID - User ID for memory isolation (default: claude-code)

set -e

API_URL="${VELD_API_URL:-http://127.0.0.1:3030}"
USER_ID="${VELD_USER_ID:-claude-code}"

# API Key - required (no hardcoded fallback for security)
if [ -z "$VELD_API_KEY" ]; then
    echo "ERROR: VELD_API_KEY environment variable not set" >&2
    exit 1
fi
...
# Send to veld API (fire and forget, don't block Claude)
